// render-check.html (no check): the Canvas 2D path (the fallback renderer; boot.js opens the game with
// ?renderer=canvas) measured layer by layer on a controlled High scene, bracketed by browser and animated-canvas
// controls. The WebGL2 renderer, the default, has its own check: render-check.html?check=gl (gl-controller.mjs).
import { stages, warmupMs, sampleMs, summarize, measuredInterval, replaceMethod, restore, assessControls } from './metrics.mjs?v=gl-default-1';
...
function isolate(kind, undo) {
  ...
  // without post-processing the scene layer (see game.js sceneCv) is copied to the visible canvas as it is
  if (kind === 'noPost') patch(g, 'post', (src) => {
    if (!src) return;
    const c = cv.getContext('2d');
    c.save(); c.setTransform(1, 0, 0, 1, 0, 0); c.globalCompositeOperation = 'copy'; c.globalAlpha = 1; c.drawImage(src, 0, 0); c.restore();
  });
  if (kind === 'noFighters') {
