...
// opts.present (?post=present): the WebGL canvas itself is shown above the game canvas, so the finished picture is
// not copied back into Canvas 2D and the drawing buffer need not be preserved; render(input, null, ...) then only
// draws into the WebGL canvas. Without it (?post=webgl) the result is copied back into `output` as before.
(function (ND) {
  'use strict';
  ND.createGpuPost = function (grain, opts) {
    const present = !!(opts && opts.present);
    const canvas = document.createElement('canvas');
    let gl;
    try { gl = canvas.getContext('webgl', { alpha: false, antialias: false, depth: false, stencil: false, preserveDrawingBuffer: !present }); }
    catch (e) { return null; }
    if (!gl) return null;
    ...
    return {
      ...
      render(input, output, strength, x, y) {
        ...
        try {
          ...
          if (!output) return true; // present mode: the WebGL canvas is the visible picture
