...
async function measure(kind, label) {
  ...
  let count = 0, begin, previous, previousElapsed = -1, previousCpu = 0, previousIndex = -1;
  let sampleStart = null;
  strip.textContent = label;
  for (;;) {
    const now = await nextFrame(); active(); begin ??= now;
    const elapsed = now - begin;
    if (previous !== undefined && previousElapsed >= warmup) {
      if (count >= 8192) throw Error('Frame telemetry capacity exceeded');
      const offset = count++ * stride;
      records[offset] = now - previous; records[offset + 1] = previousCpu; records[offset + 2] = previousIndex;
      records.set(previousDelta, offset + 3);
    }
    if (elapsed >= warmup + sample) break;
    if (elapsed >= warmup && !sampleStart) sampleStart = cacheCounts(N, g);
    ...
    previous = now; previousElapsed = elapsed; previousIndex = index;
  }
  ...
  const result = { kind, label, ...summarize(samples), renderer: g.renderVersion, gpu: g.postGpuStatus(),
...
$('rc-help').textContent = 'Moving High-quality comparison. Keep the phone sideways and this tab visible. Preparation is followed by about 52 seconds of automatic testing.';
...
$('rc-back').href = './?post=webgl&v=6'; $('rc-back').textContent = 'BACK TO GAME';
$('rc-status').textContent = 'Ready · prepared-cache comparison · High'; start.disabled = false;
start.onclick = async () => {
  if (running) return;
  ...
  report = { report: 'shadow-duel-prepared-check-v1', build: 'deterministic-cloth-hard-budget-v1', complete: false,
  ...
  try {
    ...
    report.controls = assessControls(report.stages); report.complete = true;
